...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Replace 'cookieName' and 'example.com' with the appropriate values
  document.cookie =
    "gearbox_ad_token=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=yektanet.com";
  ...
  const removeBlockedCookies = () => {
    ...
    if (cookiesBlockList) {
      for (let i = 0; i < cookiesBlockList.length; i++) {
        document.cookie = `${cookiesBlockList[i]}=false; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=yourdomain.com`;
      }
    } else {
  ...
  const addCookieConsentModal = (container) => {
    ...
    rejectButton.innerText = "ردیفه";
    rejectButton.style =
      "width: 100px; height: 40px; background-color: #fed812; color: #434343; border: none; cursor: pointer;font-weight: bold";
    rejectButton.onclick = () => {
      const Wrapper = document.querySelector(".cookie-consent-modal");
      Wrapper.style.display = "none";
      setCookie(ACCEPT_COOKIE_CONSENT, "true", 2);
    };

    const readMoreLink = document.createElement("a");
    readMoreLink.href = "#";
    readMoreLink.innerText = "مدیریت کوکی‌ها";
    readMoreLink.style =
  ...
  const setCookie = (name, value, days) => {
    if (!name || !value || !days) {
      console.error("Invalid parameters provided to setCookie function.");
      return;
    }
  ...
  console.log("cookie accepted: ", hasCookieConsent);
  ...
  if (hasCookieConsent) {
    ...
    console.log("cookie not accepted: ", hasCookieConsent);
    setCookie(ACCEPT_COOKIE_CONSENT, "true", 2);
  }
  ...
  setTimeout(() => {
    console.log("from timeout: ", hasAcceptedCookieConsent);
    if (!hasAcceptedCookieConsent) {
